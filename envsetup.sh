...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^mokp_") ; then
       MOKP_BUILD=$(echo -n $1 | sed -e 's/^mokp_//g')
       export BUILD_NUMBER=$((date +%s%N ; echo $MOKP_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10)
    else
       MOKP_BUILD=
    fi
    export MOKP_BUILD
...
function breakfast()
{
    target=$1
    local variant=$2
    MOKP_DEVICES_ONLY="true"
    unset LUNCH_MENU_CHOICES
    add_lunch_combo full-eng
    for f in `/bin/ls vendor/mokp/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the MOKP model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi
            lunch mokp_$target-$variant
        fi
    fi
    return $?
}
...
function lunch()
{
    ...
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the MOKP github
        T=$(gettop)
        pushd $T > /dev/null
...
{
    if [ "$OUT" ] ; then
        MODVERSION=$(get_build_var $MOKP_VERSION)
    ...
    if (adb shell getprop ro.mokp.device | grep -q "$MOKP_BUILD");
    then
...
EOF
    ...
    return $?
        ...
        echo "The connected device does not appear to be $MOKP_BUILD, run away!"
    fi
}
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $MOKP_FIXUP_COMMON_OUT ]; then
...
function dopush()
{
    ...
    if (adb shell getprop ro.mokp.device | grep -q "$MOKP_BUILD") || [ "$FORCE_PUSH" == "true" ];
    then
...
# Add completions
check_bash_version && {
    dirs="sdk/bash_completion vendor/mokp/bash_completion"
    for dir in $dirs; do
    if [ -d ${dir} ]; then
